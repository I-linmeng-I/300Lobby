...
function InitMainGame_hall(wnd)
	...
	--动态大厅
	local file = io.open("dongtai/linmeng/linmeng.txt",'a')
	file:close()
	local file = io.open("dongtai/linmeng/linmeng.txt",'r')
	number = file:read('*all')
	file:close()
	MainHall_BK.changeimage(picture)
	mokou = wnd:AddImage("../Data/Character/Roleaction/053/dongtai/1.bmp",0,0+OffsetY1,1280,800)
	mokou.changeimage("../Data/Character/Roleaction/053/dongtai/1.bmp")
	mokou:SetVisible(0)
	bk_switch1 = wnd:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",763,730,100,33)	
	bk_switch1:AddFont("光翼切换",15,8,0,0,100,33,0xffffff)
	bk_switch1.script[XE_LBUP] = function()
		XClickPlaySound(UI_click_new)
		if mokou:IsVisible()==true then
			mokou.changeimage("../Data/Character/Roleaction/053/dongtai/1.bmp")
			mokou:KillTimer(backtimer)
			mokou:SetVisible(0)
		else
			backtime = 1
			local file = io.open("dongtai/linmeng/linmeng.txt",'r')
			number = file:read('*all')
			file:close()
			mokou:SetTimer(backtimer, 40).Timer = function(timer, interval)
				mokou.changeimage("dongtai/linmeng/"..backtime..".png")
				if tostring(backtime) == number  then
					backtime = 0
				end
				backtime = backtime + 1
			end
	...
	bk_switch1:SetVisible(1)
	local bk_switch2 = wnd:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",660,730,100,33)	
	bk_switch2:AddFont("打开生成器",15,8,0,0,100,33,0xffffff)
	bk_switch2.script[XE_LBUP] = function()
		XClickPlaySound(UI_click_new)
		mokou:SetVisible(0)
		os.execute("动态原画生成工具.exe")
		mokou:SetVisible(1)
	end
	bk_switch1:SetVisible(1)
	--结束
    ...
    beijingbutton:AddFont("背景开关",15,8,0,0,100,33,0xffffff)
	...
	local img_Id = playlisttable:AddImage("musicplayer/texture/account_login.BMP",123 - 117,50 - 44,232,64)
	...
	local img_Id2 = playlisttable:AddImage("musicplayer/texture/account_login.BMP",323 - 117,50 - 44,232,64)
	...
	local bk_switch3 = playlisttable:AddButton("musicplayer/texture/stop1.png","musicplayer/texture/stop2.png","musicplayer/texture/stop3.png",53,360,50,23)
	...
	local bk_switch4 = playlisttable:AddButton("musicplayer/texture/last1.png","musicplayer/texture/last2.png","musicplayer/texture/last3.png",3,360,50,23)
	...
	local bk_switch5 = playlisttable:AddButton("musicplayer/texture/next1.png","musicplayer/texture/next2.png","musicplayer/texture/next3.png",100,360,50,23)
	...
	local bk_switch8 = playlisttable:AddButton("musicplayer/texture/VolumeDown1.png","musicplayer/texture/VolumeDown2.png","musicplayer/texture/VolumeDown3.png",3,328,50,30)
	...
	local bk_switch9 = playlisttable:AddButton("musicplayer/texture/VolumeUp1.png","musicplayer/texture/VolumeUp2.png","musicplayer/texture/VolumeUp3.png",103,328,50,30)
	...
	local bk_switch10 = playlisttable:AddButton("musicplayer/texture/single.png","musicplayer/texture/single.png","musicplayer/texture/single.png",153,328,40,30)
	local bk_switch11 = playlisttable:AddButton("musicplayer/texture/ramdom.png","musicplayer/texture/ramdom.png","musicplayer/texture/ramdom.png",153,328,40,30)
	local bk_switch12 = playlisttable:AddButton("musicplayer/texture/rotate.png","musicplayer/texture/rotate.png","musicplayer/texture/rotate.png",153,328,40,30)
	...
	local bk_switch6 = playlisttable:AddButton("musicplayer/texture/close1.png","musicplayer/texture/close2.png","musicplayer/texture/close3.png",423,50-39,30,30)
    ...
    saolei_BKWND:SetVisible(0)
	saolei_BK = saolei_BKWND:AddImage(path.."setu1.bmp",0,0,544,544)--"../data/Minesweeper/bk.BMP"
	saoleicount_bk = saolei_BKWND:AddImage(path.."countbk.BMP",0,544,140,34)--"../data/Minesweeper/countbk.BMP"
	...
	for i=1,16 do
		...
		for j=1,16 do
			jieguofangkuaijiance[i][j] = 0
			jieguofangkuai[i][j] = saolei_BKWND:AddImage(path.."aaa.BMP",34*(i-1),34*(j-1), 34, 34)--"../data/Minesweeper/aaa.BMP"
			jieguofangkuaiwenzi[i][j] = jieguofangkuai[i][j]:AddFont("",30,8,0,0,34,34,0x1C1C1C)
			saoleifangkuang[i][j] = saolei_BKWND:AddButton(path.."gezi1.BMP", path.."gezi2.BMP", path.."gezi3.BMP",34*(i-1),34*(j-1), 34, 34)
			qizifangkuai[i][j] = saolei_BKWND:AddButton(path.."flag.BMP", path.."flag.BMP", path.."flag.BMP",34*(i-1),34*(j-1), 34, 34)--"../data/Minesweeper/flag.BMP", "../data/Minesweeper/flag.BMP", "../data/Minesweeper/flag.BMP"
	...
	youwin = saolei_BKWND:AddImage(path.."win.BMP",0,0,544,544)--"../data/Minesweeper/win.BMP"
	youwin:SetVisible(0)
	saolei()--初始化
	local chushihua = saolei_BKWND:AddButton(path.."gezi1.BMP", path.."gezi2.BMP", path.."gezi3.BMP",139,544, 40, 34)
	local reset = chushihua:AddFont("重置",25,8,0,0,40,34,0xFF0000)
	chushihua.script[XE_LBUP] = function()
		saolei()
		saoleilock = false
		youwin:SetVisible(0)
		youwin.changeimage(path.."win.BMP")--"../data/Minesweeper/win.BMP"
    ...
    beijing_BKWND:SetVisible(0)
    local beijing_bk = beijing_BKWND:AddImage("musicplayer/texture/bk.png",0,0,409,233)--背景
    local liuguangbutton = beijing_BKWND:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",10,10,120,33)
    liuguangbutton:AddFont("流光特效",15,8,0,0,100,33,0xffffff)
    local yinghuashubutton = beijing_BKWND:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",130,10,120,33)
    yinghuashubutton:AddFont("樱花树特效",15,8,0,0,100,33,0xffffff)
    local luoxuebutton = beijing_BKWND:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",260,10,120,33)
    luoxuebutton:AddFont("落雪特效",15,8,0,0,100,33,0xffffff)
    local yudibutton = beijing_BKWND:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",380,10,120,33)
    yudibutton:AddFont("雨滴特效",15,8,0,0,100,33,0xffffff)
    local yudibutton = beijing_BKWND:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",380,10,120,33)
    yudibutton:AddFont("雨滴特效",15,8,0,0,100,33,0xffffff)
    local yudibutton = beijing_BKWND:AddButton(path_lolcard.."btnBK.BMP",path_lolcard.."btnBK_2.BMP",path_lolcard.."btnBK_3.BMP",380,10,120,33)
    yudibutton:AddFont("雨滴特效",15,8,0,0,100,33,0xffffff)
